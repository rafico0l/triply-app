-- ============================================================
-- Corrective 3: Complete RETURNS TABLE ambiguity fix
-- ============================================================
-- Fixes:
-- 1. RETURNING id bare reference in create_settlement_request
-- 2. Invalid SET alias prefixes (sr.status, ra.status, s.amount_minor)
--    in all three RPCs
-- INSERT target columns remain bare (required by PostgreSQL).
-- All other expression contexts already use table aliases.

-- ── create_settlement_request ────────────────────────────────
-- Collisions found:
--   RETURNING id INTO v_request_id  → bare "id" collides with
--     RETURNS TABLE output variable "id"
-- Fix: add AS sr alias to INSERT, qualify RETURNING
...
BEGIN
  ...
  INSERT INTO public.settlement_requests AS sr (
...
-- Collisions found:
--   SET ra.status = ...        → invalid SET alias prefix
--   SET sr.status = ...        → invalid SET alias prefix
--   SET s.amount_minor = ...   → invalid SET alias prefix
-- All other expression contexts already use table aliases correctly.
