-- ============================================================
-- Corrective: Fix ambiguous column references in settlement RPCs
-- ============================================================
-- Fixes PostgreSQL error 42702: "column reference trip_id is
-- ambiguous" in create_settlement_request.
-- All three RPCs audited and qualified where needed.
-- Original schema/RLS untouched.

-- ── create_settlement_request ────────────────────────────────
-- INSERT column names matched PL/pgSQL variable names
-- (trip_id, settlement_id, requester_id, etc.).
-- Qualify with table alias to eliminate ambiguity.
...
BEGIN
  ...
  INSERT INTO public.settlement_requests (
    sr.trip_id, sr.settlement_id, sr.request_type, sr.requester_id,
    sr.original_amount_minor, sr.proposed_amount_minor
  )
  VALUES (
  ...
  FOREACH v_approver_id IN ARRAY v_approver_ids LOOP
    INSERT INTO public.settlement_request_approvals (ra.request_id, ra.approver_id)
...
-- Audit: all column references already qualified via table aliases
-- (sr, ra, s) or use PL/pgSQL variables (v_request, v_settlement).
-- Replaced with qualified form for consistency and safety.
...
-- Audit: all column references already qualified.
-- Replaced with qualified form for consistency and safety.
