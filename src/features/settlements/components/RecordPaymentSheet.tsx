    ...
    <>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[480px] bg-white rounded-t-[24px] z-50 safe-bottom" style={{ animation: "sheetUp 240ms cubic-bezier(0.32,0.72,0,1)" }}>
        ...
        <div className="px-5 pt-2 pb-2 flex items-center justify-between">
          <h2 className="text-[17px] font-700 text-[#0F172A]">Record payment</h2>
          <button onClick={onClose} className="pressable text-[14px] font-600 text-[#94A3B8]">Cancel</button>
        </div>
        <div className="px-5 pb-6 space-y-4">
          {/* From / To — compact horizontal relationship */}
          <div className="bg-[#F8FAFC] rounded-[14px] border border-[#E1E7EF] overflow-hidden">
            ...
            >
              <div className="flex items-center gap-2 flex-1 min-w-0">
                {from ? (
                  <><Avatar member={from} size="sm" /><span className="text-[14px] font-600 text-[#0F172A] truncate">{from.isMe ? "You" : from.name}</span></>
                ) : (
                  <span className="text-[14px] font-500 text-[#C9D4DF]">Select payer…</span>
                )}
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <span className="text-[11px] font-700 text-[#94A3B8] uppercase tracking-wide">From</span>
                {isManual && <span className="text-[#C9D4DF]"><IconChevronRight size={14} /></span>}
              </div>
            </button>
            <div className="flex items-center justify-center py-1 border-b border-[#E1E7EF]">
            ...
            >
              <div className="flex items-center gap-2 flex-1 min-w-0">
                {to ? (
                  <><Avatar member={to} size="sm" /><span className="text-[14px] font-600 text-[#0F172A] truncate">{to.isMe ? "You" : to.name}</span></>
                ) : (
                  <span className="text-[14px] font-500 text-[#C9D4DF]">Select receiver…</span>
                )}
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                <span className="text-[11px] font-700 text-[#94A3B8] uppercase tracking-wide">To</span>
                {isManual && <span className="text-[#C9D4DF]"><IconChevronRight size={14} /></span>}
              </div>
            </button>
          </div>
          ...
          <p className="text-[12px] font-500 text-[#94A3B8] leading-relaxed text-center">
            Triply only records this payment. No money is transferred through Triply.
          </p>
